...
class MonkeysState(GameState):
    def __init__(self, L: int, x1: int, x2: int, banana: int, step: int, step_limit: int, player_turn: int):
    ...
    def available_actions(self) -> Set[MonkeysAction]:
        return set([MonkeysAction.LEFT, MonkeysAction.RIGHT, MonkeysAction.GRAB, MonkeysAction.NOTHING])
    ...
    def result(self, action: MonkeysAction) -> MonkeysState:
        ...
        # NOTHING action
        if action == MonkeysAction.NOTHING: return new_state
        
        # GRAB action
        if action == MonkeysAction.GRAB:
            # Case 1: The agent is not under the banana
            if self.xs[player] != self.banana: return new_state
            else:
        ...
        if action == MonkeysAction.LEFT:
            ...
            if self.xs[player] == 0: return new_state
            
            # Monkey is below the other: no change
            if self.monkey_up[1 - player]: return new_state 
        ...
        if action == MonkeysAction.RIGHT:
            ...
            if self.xs[player] == self.L - 1: return new_state
...
class MonkeysGame(Game):
    def __init__(self, L: int, x1_init: int, x2_init: int, banana: int, step_limit: int):
    ...
    def perform_action(self, action: MonkeysAction):
        ...
        if self.state.is_goal():
            self.done = True
            if self.state.grab:
                print(f"== Player {1 - self.state.player_turn + 1} grabbed the banana! ==")
            else:
